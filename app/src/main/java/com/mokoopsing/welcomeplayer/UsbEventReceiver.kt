    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (isCarLifeConnection(intent)) {
            ...
            val reason = "USB event did not match CarLife: " +
                ...
                "configured=${intent.getBooleanExtra(EXTRA_CONFIGURED, false)}, " +
                "compositeCarMtp=${intent.getBooleanExtra(EXTRA_CAR_MTP, false)}, " +
                "compositeCarMtpKey=${intent.hasExtra(EXTRA_CAR_MTP)}, " +
                "whitelistDevice=${intent.getBooleanExtra(EXTRA_WHITELIST_DEVICE, false)}, " +
                "whitelistDeviceKey=${intent.hasExtra(EXTRA_WHITELIST_DEVICE)}"
            Log.i(TAG, reason)
            publishLog(context, reason)
        }
    }

    private fun publishLog(context: Context, line: String) {
